...
function successFun(data){
  if(IFSCommonMethod.isNotBlank(data.lists)){
    ...
    for(var i=0;i<list.length;i++){
      if(list[i].txnStat=="00"){
        type=' class="pull-right">未提交';
      }else if (list[i].txnStat=="01"){
        type=' class="pull-right">待复核';
      }else if(list[i].txnStat=="02"){
        type='class="pull-right ">已复核';
      }else if(list[i].txnStat=="03"){
        type='class="pull-right">已取消';
      }else if(list[i].txnStat=="04"){
        type='class="pull-right">撤回待复核';
      }else if(list[i].txnStat=="05"){
        type='class="pull-right">已撤回';
      }else if(list[i].txnStat=="10"){
        type='class="pull-right">待签收';
      }else if(list[i].txnStat=="11"){
        type='class="pull-right">已签收';
      }else if(list[i].txnStat=="12"){
        type='class="pull-right">已驳回';
      }else if(list[i].txnStat=="13"){
        type='class="pull-right">放款中';
      ...
      html.push('<li  class="item" data-id="'+list[i].id+'" data-name="'+list[i].approveStat+'" onclick="progressData(this)">\
        <div class="list-data-item clearfix"><span class="pull-left">'+list[i].custNm+'</span><span class="pull-right ">'+IFSCommonMethod.formatMoney(list[i].disctAmt)+'</span></div>\
